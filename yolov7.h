...
public:
    ...
    void Infer(cv::Mat& bgr, vector<TargetBox>& outputs);
    ~Yolo();
...
void Yolo::Infer(cv::Mat& bgr, vector<TargetBox>& outputs)
{
    // 准备变量
    cv::Mat croped; Mat pr_img;
    ...
    // resize 和格式转换
    cv::resize(croped, pr_img, cv::Size{ iW, iH });
    cv::cvtColor(pr_img, pr_img, cv::COLOR_BGR2RGB);
    pr_img.convertTo(pr_img, CV_32FC3, 1.f / 255.f);
    Mat input_blob_nhwc = cv::dnn::blobFromImage(pr_img);
    ...
    for (size_t i = 0; i < num_dets[0]; i++) {
        ...
        outputs.push_back(TargetBox{ x0, y0, x1, y1 });
    }
}
